import React, { useContext, useEffect } from 'react';
import { cartContext } from '../context/cart-context';



export const Instagram = () => {
    return(
        <svg id="InstagramLogo" xmlns="http://www.w3.org/2000/svg" width="44.991" height="45" viewBox="0 0 44.991 45" fill="#fff">
            <path id="Path_1" data-name="Path 1" d="M45.326,13.23A16.443,16.443,0,0,0,44.28,7.771,11.527,11.527,0,0,0,37.7,1.187,16.486,16.486,0,0,0,32.237.141C29.828.027,29.063,0,22.954,0s-6.875.027-9.275.132A16.45,16.45,0,0,0,8.218,1.178,10.98,10.98,0,0,0,4.23,3.78,11.079,11.079,0,0,0,1.635,7.762,16.487,16.487,0,0,0,.589,13.221C.475,15.631.449,16.395.449,22.5s.026,6.875.131,9.274a16.442,16.442,0,0,0,1.046,5.459A11.526,11.526,0,0,0,8.21,43.822a16.488,16.488,0,0,0,5.459,1.046c2.4.106,3.164.132,9.274.132s6.874-.026,9.274-.132a16.439,16.439,0,0,0,5.459-1.046,11.511,11.511,0,0,0,6.585-6.584,16.5,16.5,0,0,0,1.046-5.459c.106-2.4.132-3.164.132-9.274s-.009-6.874-.115-9.274ZM41.273,31.6a12.334,12.334,0,0,1-.773,4.176,7.465,7.465,0,0,1-4.273,4.273,12.379,12.379,0,0,1-4.176.773c-2.374.106-3.086.132-9.09.132s-6.725-.026-9.09-.132A12.328,12.328,0,0,1,9.7,40.052a6.925,6.925,0,0,1-2.585-1.679,7,7,0,0,1-1.679-2.585,12.379,12.379,0,0,1-.773-4.176c-.106-2.374-.132-3.086-.132-9.09s.026-6.725.132-9.09a12.328,12.328,0,0,1,.773-4.176A6.84,6.84,0,0,1,7.121,6.672,6.985,6.985,0,0,1,9.706,4.993a12.386,12.386,0,0,1,4.176-.773c2.374-.106,3.086-.132,9.09-.132s6.725.027,9.09.132a12.334,12.334,0,0,1,4.176.773,6.92,6.92,0,0,1,2.585,1.679A6.994,6.994,0,0,1,40.5,9.257a12.385,12.385,0,0,1,.773,4.176c.106,2.374.132,3.086.132,9.09S41.379,29.23,41.273,31.6Zm0,0" transform="translate(-0.449)"/>
            <path id="Path_2" data-name="Path 2" d="M134.7,124.5a9.75,9.75,0,1,0,9.753,9.75A9.751,9.751,0,0,0,134.7,124.5Zm0,16.075a6.325,6.325,0,1,1,6.327-6.325,6.326,6.326,0,0,1-6.327,6.325Zm0,0" transform="translate(-112.196 -111.745)"/>
            <path id="Path_3" data-name="Path 3" d="M372.752,93.753A5.152,5.152,0,1,1,367.6,88.6a5.151,5.151,0,0,1,5.148,5.153Zm0,0" transform="translate(-334.569 -81.777)"/>
        </svg>
    )
}

export const Twitter = () => {
    return(
        <svg xmlns="http://www.w3.org/2000/svg" width="55.385" height="45" viewBox="0 0 55.385 45" fill="#fff">
            <g id="twitter" transform="translate(0 -48)">
                <g id="Group_2" data-name="Group 2" transform="translate(0 48)">
                <path id="Path_2" data-name="Path 2" d="M55.385,53.327a23.672,23.672,0,0,1-6.542,1.793,11.291,11.291,0,0,0,5-6.276,22.691,22.691,0,0,1-7.2,2.748A11.354,11.354,0,0,0,27,59.357a11.691,11.691,0,0,0,.263,2.589A32.139,32.139,0,0,1,3.856,50.07,11.357,11.357,0,0,0,7.345,65.245a11.213,11.213,0,0,1-5.13-1.4v.125a11.407,11.407,0,0,0,9.1,11.157,11.333,11.333,0,0,1-2.977.374,10.041,10.041,0,0,1-2.15-.194,11.462,11.462,0,0,0,10.61,7.91A22.814,22.814,0,0,1,2.717,88.06,21.268,21.268,0,0,1,0,87.9,31.966,31.966,0,0,0,17.418,93c20.894,0,32.317-17.308,32.317-32.31,0-.5-.017-.987-.042-1.468A22.652,22.652,0,0,0,55.385,53.327Z" transform="translate(0 -48)"/>
                </g>
            </g>
        </svg>
    )
}

export const Youtube = () => {
    return(
        <svg xmlns="http://www.w3.org/2000/svg" width="64.271" height="45" viewBox="0 0 64.271 45" fill="#fff">
            <path id="youtube" d="M62.947.959a8.052,8.052,0,0,0-5.665-5.665C52.253-6.082,32.135-6.082,32.135-6.082s-20.117,0-25.147,1.324A8.215,8.215,0,0,0,1.324.959C0,5.988,0,16.418,0,16.418S0,26.9,1.324,31.877a8.053,8.053,0,0,0,5.665,5.665c5.082,1.377,25.147,1.377,25.147,1.377s20.117,0,25.147-1.324a8.052,8.052,0,0,0,5.665-5.665c1.323-5.029,1.323-15.459,1.323-15.459S64.324,5.988,62.947.959ZM25.73,26.053V6.783l16.729,9.635Zm0,0" transform="translate(-0.001 6.082)"/>
        </svg>
    )
}

export const Cart = () => {

    const [cartIsOpen, setCartIsOpen] = useContext(cartContext);
     
    return(
        <svg xmlns="http://www.w3.org/2000/svg" width="28.433" height="24.99" viewBox="0 0 28.433 24.99" id="cart" onClick={() => setCartIsOpen(prev => !prev)}>
            <g id="shopping-cart" transform="translate(0)">
                <g id="Group_5" data-name="Group 5" transform="translate(0 0)">
                    <path id="Path_3" data-name="Path 3" d="M9.161,16.661H24.268a.833.833,0,0,0,.8-.6L28.4,4.394a.833.833,0,0,0-.8-1.062H7.24l-.6-2.68A.833.833,0,0,0,5.831,0h-5a.833.833,0,1,0,0,1.666h4.33L8.17,15.2a2.5,2.5,0,0,0,.993,4.792H24.268a.833.833,0,1,0,0-1.666H9.163a.833.833,0,0,1,0-1.666ZM26.5,5l-2.856,10H9.831L7.61,5Zm0,0" transform="translate(0 0)" fill="#fff"/>
                    <path id="Path_4" data-name="Path 4" d="M150,362.5a2.5,2.5,0,1,0,2.5-2.5A2.5,2.5,0,0,0,150,362.5Zm2.5-.833a.833.833,0,1,1-.833.833A.834.834,0,0,1,152.5,361.666Zm0,0" transform="translate(-141.67 -340.008)" fill="#fff"/>
                    <path id="Path_5" data-name="Path 5" d="M362,362.5a2.5,2.5,0,1,0,2.5-2.5A2.5,2.5,0,0,0,362,362.5Zm2.5-.833a.833.833,0,1,1-.833.833A.834.834,0,0,1,364.5,361.666Zm0,0" transform="translate(-341.897 -340.008)" fill="#fff"/>
                </g>
            </g>
        </svg>
    )
}

export const Hamburger = ({reveal}) => {
    return(
        <svg xmlns="http://www.w3.org/2000/svg" width="26" height="19" viewBox="0 0 26 19" id="hamburger" onClick={() => reveal()}>
            <g id="Hamburger_Menu" data-name="Hamburger Menu" transform="translate(-20 -14)">
                <rect id="Rectangle_37" data-name="Rectangle 37" width="26" height="3" rx="1.5" transform="translate(20 14)" fill="#fff"/>
                <rect id="Rectangle_38" data-name="Rectangle 38" width="26" height="3" rx="1.5" transform="translate(20 30)" fill="#fff"/>
                <rect id="Rectangle_39" data-name="Rectangle 39" width="26" height="3" rx="1.5" transform="translate(20 22)" fill="#fff"/>
            </g>
        </svg>
    )
}

export const Exit = ({color, hide = () => { return } }) => {

    return(
        <svg xmlns="http://www.w3.org/2000/svg" width="20.506" height="20.506" viewBox="0 0 20.506 20.506" id="exit" onClick={() => hide()}>
            <g id="Exit" transform="translate(-99.747 -14.247)">
                <rect id="Rectangle_92" data-name="Rectangle 92" width="26" height="3" rx="1.5" transform="translate(101.868 14.247) rotate(45)" fill={color}/>
                <rect id="Rectangle_93" data-name="Rectangle 93" width="26" height="3" rx="1.5" transform="translate(99.747 32.632) rotate(-45)" fill={color}/>
            </g>
        </svg>
    )
}

export const Plus = (props) => {
    return(
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 25 25" fill="#2a2a2a"> 
            <path 
                d="M12 0c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm6 13h-5v5h-2v-5h-5v-2h5v-5h2v5h5v2z"
            />
        </svg>
    )
}

export const Edit = (props) => {
    return(
        <svg enable-background="new 0 0 29 30" 
            height="30px" id="Layer_1" 
            version="1.1" viewBox="0 0 29 30" 
            width="29px">
            <g>
                <rect fill="#FFF" 
                    height="22.68" 
                    transform="matrix(-0.7071 -0.7072 0.7072 -0.7071 10.4473 37.3449)" 
                    width="10.846" x="7.536" y="5.169"
                />
                <path 
                    d="M27.638,3.996l-2.46-2.459c-1.357-1.358-3.56-1.358-4.917,0l-1.725,1.724l7.67,7.669l1.725-1.724   C29.288,7.848,28.997,5.354,27.638,3.996z" 
                    fill="#FFF"
                />
                <polygon 
                    fill="#FFF" 
                    points="0,30 7.088,30 0,22.28 "
                />
            </g>
        </svg>
    )
}

export const Loader = ({color}) => {

    const loadingInStyle = {
        width: '100px',
        height: '100px',
        margin: '2.5rem auto',
    }
    return(
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" style={loadingInStyle} className="loader">
            <path fill="none" d="M0 0h24v24H0z"/>
            <path d="M18.364 5.636L16.95 7.05A7 7 0 1 0 19 12h2a9 9 0 1 1-2.636-6.364z" fill={color}/>
        </svg>
    )
}